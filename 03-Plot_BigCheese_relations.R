...
p <- ggdraw() +
  draw_plot(p.DOS,        x = 0,    y = 0, width = 0.51, height = 1) +
  draw_plot(p.temp.prec7, x = 0.51, y = 0, width = 0.49, height = 1)

save_plot("Figure_BigCheese_time-varying.jpg", p, ncol = 1.5, nrow = 1, dpi = 600)
...
fill.spp <- ifelse(supp, colors.spp, "white")
linetype.spp <- ifelse(supp, "solid", "dashed")
dat.plot <- dat.plot.pastos_cv.BAIS %>%
  mutate(spp = "BAIS") %>%
  bind_rows(dat.plot.pastos_cv.GRSP %>%
              mutate(spp = "GRSP")) %>%
  mutate(spp = factor(spp, levels = c("BAIS", "GRSP"))) %>%
  mutate(x = x * 100)
p.pastos_cv.PSR <- ggplot(dat.plot, aes(x = x, y = PSR.md)) +
  geom_ribbon(aes(ymin = PSR.lo, ymax = PSR.hi, fill = spp, color = spp, linetype = spp), alpha = 0.3) +
  geom_line(aes(color = spp), size = 1) +
  scale_color_manual(values = colors.spp) +
  scale_fill_manual(values = fill.spp) +
  ...
  ylim(0, 1) +
  guides(color = F, fill = F, linetype = F) +
  xlab("CV Grass cover 5m (%)") + ylab(NULL)

# Tumbleweed #
supp <- c(F, T)
fill.spp <- ifelse(supp, colors.spp, "white")
linetype.spp <- ifelse(supp, "solid", "dashed")
dat.plot <- dat.plot.salsola.BAIS %>%
  mutate(spp = "BAIS") %>%
  bind_rows(dat.plot.salsola.GRSP %>%
              mutate(spp = "GRSP")) %>%
  mutate(spp = factor(spp, levels = c("BAIS", "GRSP")))
p.salsola.PSR <- ggplot(dat.plot, aes(x = x, y = PSR.md)) +
  ...
  guides(color = F, fill = F, linetype = F) +
  xlab("Tumbleweed cover 5m (%)") + ylab(NULL)
...
fill.spp <- ifelse(supp, colors.spp, "white")
linetype.spp <- ifelse(supp, "solid", "dashed")
dat.plot <- dat.plot.NDVI.BAIS %>%
  mutate(spp = "BAIS") %>%
  bind_rows(dat.plot.NDVI.GRSP %>%
              mutate(spp = "GRSP")) %>%
  mutate(spp = factor(spp, levels = c("BAIS", "GRSP")))
p.NDVI.PSR <- ggplot(dat.plot, aes(x = x, y = PSR.md)) +
  geom_ribbon(aes(ymin = PSR.lo, ymax = PSR.hi, fill = spp, color = spp, linetype = spp), alpha = 0.3,
              show.legend = F) +
  geom_line(aes(color = spp), size = 1) +
  scale_color_manual(values = colors.spp) +
  ...
  #guides(color = F, fill = F, linetype = F) +
  xlab("Greenness (NDVI)") + ylab(NULL)

# Put everything together
p <- ggdraw() +
  draw_plot(p.hierbas.PSR,           x = 0,    y = 0.6667, width = 0.25, height = 0.3333) +
  draw_plot(p.hierbas_cv.PSR,        x = 0.25, y = 0.6667, width = 0.25, height = 0.3333) +
  draw_plot(p.pastos_cv.PSR,         x = 0.5,  y = 0.6667, width = 0.25, height = 0.3333) +
  draw_plot(p.otra.PSR,              x = 0.75, y = 0.6667, width = 0.25, height = 0.3333) +

  draw_plot(p.salsola.PSR,           x = 0,    y = 0.3333, width = 0.25, height = 0.3333) +
  draw_plot(p.Shrub_All_5m.PSR,      x = 0.25, y = 0.3333, width = 0.25, height = 0.3333) +
  draw_plot(p.Shrub_All_5m_CV.PSR,   x = 0.5,  y = 0.3333, width = 0.25, height = 0.3333) +
  draw_plot(p.Shrub_All_500m_CV.PSR, x = 0.75, y = 0.3333, width = 0.25, height = 0.3333) +
  
  draw_plot(p.peso.PSR,              x = 0,    y = 0,      width = 0.25, height = 0.3333) +
  draw_plot(p.prey.PSR,              x = 0.25, y = 0,      width = 0.25, height = 0.3333) +
  draw_plot(p.LOSH.PSR,              x = 0.5,  y = 0,      width = 0.25, height = 0.3333) +
  draw_plot(p.NDVI.PSR,              x = 0.75, y = 0,      width = 0.25, height = 0.3333)
p <- ggdraw() +
...
#   scale_color_manual(labels = c("0.00006", "16"), values = c("#999999", "#D55E00")) + #009E73
#   #scale_fill_manual(labels = c("0.008", "16"), values = c("#999999", "#D55E00")) + #009E73
...
#   geom_errorbar(data = dat.plot.noshrubs, aes(x = x1, ymin = PSR.lo, ymax = PSR.hi), width = 0.01) +
#   geom_point(data = dat.plot.noshrubs, aes(x = x1, y = PSR.md), size = 2) +
#   scale_color_manual(labels = c("0.00002", "18"), values = c("#999999", "#D55E00")) +  #009E73
#   annotate("text", x = -0.03, y = 0.3, label = "No shrubs", size = 4, angle = 90) +
#   geom_vline(xintercept = 0.055, linetype = "dashed") +
...
# 
# p <- ggdraw() +
#   draw_plot(p.heat.BAIS,   x = 0,   y = 0, width = 0.5,  height = 0.95) +
#   draw_plot(p.heat.GRSP,   x = 0.5, y = 0, width = 0.5,  height = 0.95) +
#   draw_plot_label(c("Baird's Sparrow", "Grasshopper Sparrow"),
#                   x = c(0.07, 0.52), y = c(1, 1))
# save_plot("Figure_BigCheese_interactions.jpg", p, ncol = 2.5, nrow = 1, dpi = 600)
