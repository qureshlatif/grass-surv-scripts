...
vars.quad.DSR <- c(T, F)
vars.to.plot.PSR <- c("hierbas", "hierbas_cv", "pastos_cv", "salsola", "otra",
                      "Shrub_All_5m", "Shrub_All_5m_CV", "Shrub_All_500m_CV",
                      "peso", "prey", "LOSH", "NDVI")
vars.quad.PSR <- c(T, F, F, T, T,
                   T, F, F,
                   F, F, F, F)
...
rm(mod, B0, B1, B2, B.DOS, B.DOS2, B.temp.min, B.temp.prec7, x, z, z.arr, z.DOS,
   z.DOS2, z.temp.min, z.temp.prec7, DSR, PSR, st, end, dat.plot, data.spp, v) # B12, B22, B3, z1, z2, z1.arr, z2.arr, 
